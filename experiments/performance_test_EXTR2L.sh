...
COMMAND="../tool/build/src/demiurge-bin -b load -s min_api -q blo_dep_api -c learn -m 2 --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_EXTR2M_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_EXTR2M_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
