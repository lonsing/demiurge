...
COMMAND="../tool/build/src/demiurge-bin -r -b templ -q blo_dep_api --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_TB_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_TB_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
