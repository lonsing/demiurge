...
COMMAND="../tool/build/src/demiurge-bin -r -b learn_sat -m 3 -s pic_api --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_S3P_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_S3P_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
