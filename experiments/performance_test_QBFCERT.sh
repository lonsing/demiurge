...
COMMAND="../tool/build/src/demiurge-bin -b load -s lin_api -c qbfcert --print=L"

# Maybe change the following line to point to GNU time:
GNU_TIME="/usr/bin/time"
MODEL_CHECKER="$DEMIURGETP/lingeling/blimc"
IC3_CHECKER="/home/rkoenighofer/data/svns/collab-qbf_synth/ic3ref/IC3"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_QBFCERT_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_QBFCERT_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
