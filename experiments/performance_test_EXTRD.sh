...
COMMAND="../tool/build/src/demiurge-bin -b load -s min_api -q blo_dep_api -c learn -m 21 --print=DERWIL"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_EXTRD21M_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_EXTRD21M_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
...
do
     ...
     echo "=====================  $file_name.aag =====================" 1>> $RES_TXT_FILE 2>&1
