...
COMMAND="../branches/interpol/tool/build/src/demiurge-bin -b load -s min_api -c interpol -n 2 --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_extr_I2_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_extr_I2_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
