...
COMMAND="../branches/interpol/tool/build/src/demiurge-bin -b load -s min_api -c interpol -n 0 --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_extr_I0_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_extr_I0_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
