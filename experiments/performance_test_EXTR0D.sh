...
COMMAND="../tool/build/src/demiurge-bin -b load -s min_api -q depqbf_api -c learn -m 0 --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_EXTR0D_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_EXTR0D_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
