...
COMMAND="../tool/build/src/demiurge-bin -r -b learn_sat -m 1 -s min_api --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_SGM_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_SGM_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
