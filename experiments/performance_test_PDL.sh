...
COMMAND="../tool/build/src/demiurge-bin -r -b ifm -s lin_api --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_PDL_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_PDL_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}

ulimit -m ${MEMORY_LIMIT} -t ${TIME_LIMIT}
