...
COMMAND="../tool/build/src/demiurge-bin -b load -s lin_api -q blo_dep_api -c learn -m 15 --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_EXTR15L_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_EXTR15L_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
