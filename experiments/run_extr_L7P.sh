...
COMMAND="../tool/build/src/demiurge-bin -b load -s pic_api -c learn -n 7 --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_extr_L7P_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_extr_L7P_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
