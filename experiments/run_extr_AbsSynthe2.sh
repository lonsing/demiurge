...
COMMAND="python ../abssynthe2/abssynthe.py"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_extr_AbsSynthe_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_extr_AbsSynthe_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
...
do
     ...
     ${GNU_TIME} --quiet --output=${RES_TXT_FILE} -a -f "Synthesis time: %e sec (Real time) / %U sec (User CPU time)" ${COMMAND} $infile_path -o $outfile_path >> ${RES_TXT_FILE}
     exit_code=$?
