...
COMMAND="../tool/build/src/demiurge-bin -b load -s min_api -c learn -n 26 --print=L"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_extr_L26M_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_extr_L26M_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
