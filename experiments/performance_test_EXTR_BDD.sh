...
COMMAND="python ../bdd_tool/aisy_group4_rel.py"
...
TIMESTAMP=`date +%s`
RES_TXT_FILE="${DIR}results/results_EXTR_BDD_${TIMESTAMP}.txt"
RES_DIR="${DIR}results/results_EXTR_BDD_${TIMESTAMP}/"
mkdir -p "${DIR}results/"
mkdir -p ${RES_DIR}
...
do
     ...
     ${GNU_TIME} --quiet --output=${RES_TXT_FILE} -a -f "Synthesis time: %e sec (Real time) / %U sec (User CPU time)" ${COMMAND} -o $outfile_path $infile_path >> ${RES_TXT_FILE}
     exit_code=$?
