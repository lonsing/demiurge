...
// -------------------------------------------------------------------------------------------
///
/// @brief Extracts literals that are of a given type from a cube or clause.
///
/// This method takes as argument a cube or a clause (a set of literals), and a kind of
/// variables. It returns the same set of literals with all variables of other kind removed.
///
/// @param cube_or_clause The cube or clause from which only the desired literals should be
///        extracted.
/// @param kind The kind of literals that should be extracted. Literals of other kinds will
///        be discarded.
/// @return The literals of cube_or_clause that are of a given type. That is, the
///         returned vector is a subset of cube_or_clause, where all elements are of the
///         desired kind.
  static vector<int> extract(const vector<int> &cube_or_clause, VarInfo::VarKind kind);
