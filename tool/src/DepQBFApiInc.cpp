...
// -------------------------------------------------------------------------------------------
/// @file DepQBFApiInc.cpp
/// @brief Contains the definition of the class DepQBFApiInc.
// -------------------------------------------------------------------------------------------
...
// -------------------------------------------------------------------------------------------
DepQBFApiInc::DepQBFApiInc(bool use_bloqqer) :
...
// -------------------------------------------------------------------------------------------
DepQBFApiInc::~DepQBFApiInc()
{
  clearIncrementalSession();
}

// -------------------------------------------------------------------------------------------
bool DepQBFApiInc::isSat(const vector<pair<VarInfo::VarKind, Quant> > &quantifier_prefix,
                      const CNF& cnf)
{
  if(use_bloqqer_)
  {
...
// -------------------------------------------------------------------------------------------
bool DepQBFApiInc::isSat(const vector<pair<vector<int>, Quant> > &quantifier_prefix,
                      const CNF& cnf)
{
  if(use_bloqqer_)
  {
...
// -------------------------------------------------------------------------------------------
bool DepQBFApiInc::isSatModel(const vector<pair<VarInfo::VarKind, Quant> > &quantifier_prefix,
                           const CNF& cnf,
                           vector<int> &model)
{
  if(use_bloqqer_)
...
// -------------------------------------------------------------------------------------------
bool DepQBFApiInc::isSatModel(const vector<pair<vector<int>, Quant> > &quantifier_prefix,
                           const CNF& cnf,
                           vector<int> &model)
{
  if(use_bloqqer_)
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::startIncrementalSession(
    const vector<pair<VarInfo::VarKind, Quant> > &quantifier_prefix)
{
#ifdef DEPQBF_CORE_AVAILABLE
  if(inc_solver_ != NULL)
    qdpll_delete(inc_solver_);
  inc_solver_ = qdpll_create();
  qdpll_configure(inc_solver_, const_cast<char*>("--no-lazy-qpup"));
  CNF empty;
...
#else
  MASSERT(false, "This feature is only available with an experimental version of DepQBF. ");
#endif
}
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::clearIncrementalSession()
{
#ifdef DEPQBF_CORE_AVAILABLE
  if(inc_solver_ != NULL)
  {
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::incAddCNF(const CNF &cnf)
{
#ifdef DEPQBF_CORE_AVAILABLE
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::incAddClause(const vector<int> &clause)
{
#ifdef DEPQBF_CORE_AVAILABLE
  MASSERT(inc_solver_ != NULL, "No open session.");
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::incAddCube(const vector<int> &cube)
{
#ifdef DEPQBF_CORE_AVAILABLE
  MASSERT(inc_solver_ != NULL, "No open session.");
...
// -------------------------------------------------------------------------------------------
bool DepQBFApiInc::incIsSat(const vector<int> &assumptions)
{
#ifdef DEPQBF_CORE_AVAILABLE
  MASSERT(inc_solver_ != NULL, "No open session.");
...
// -------------------------------------------------------------------------------------------
bool DepQBFApiInc::incIsSatModelOrCore(const vector<int> &assumptions,
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::initBloqqer(const vector<pair<VarInfo::VarKind, Quant> > &quantifier_prefix,
                            const CNF& cnf)
{
  VarManager &VM = VarManager::instance();
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::initBloqqer(const vector<pair<vector<int>, Quant> > &quantifier_prefix,
                            const CNF& cnf)
{
  int max_cnf_var = 0;
  for(size_t level = 0; level < quantifier_prefix.size(); ++level)
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::initDepQBF(QDPLL *solver,
                           const vector<pair<VarInfo::VarKind, Quant> > &quantifier_prefix,
                           const CNF& cnf)
{
  VarManager &VM = VarManager::instance();
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::extractCore(QDPLL *solver, vector<int> &core)
{
#ifdef DEPQBF_CORE_AVAILABLE
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::debugCheckBloqqerVerdict(int bloqqer_verdict,
                              const vector<pair<VarInfo::VarKind, Quant> > &quantifier_prefix,
                              const CNF& cnf)
{
#ifndef NDEBUG
  DepQBFApiInc checker(false);
  bool check_sat = checker.isSat(quantifier_prefix, cnf);
  if(((!check_sat) && (bloqqer_verdict == 10)) || (check_sat && (bloqqer_verdict == 20)))
  {
    L_ERR("Error in DepQBFApiInc when using Bloqqer.");
    ...
    bloqqer_print();
    MASSERT(false, "Error in DepQBFApiInc when using Bloqqer.");
  }
#endif
}
...
// -------------------------------------------------------------------------------------------
void DepQBFApiInc::debugCheckBloqqerModel(const vector<int> &model,
                            const vector<pair<VarInfo::VarKind, Quant> > &quantifier_prefix,
                            const CNF& cnf)
{
#ifndef NDEBUG
